...
from std_msgs.msg import String  # AJOUT

# IMPORTS RELATIFS CORRIGES
...
class ScoutWork(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('ransac_dist_th', 0.1)
        self.declare_parameter('ransac_min_inliers', 30)
        self.declare_parameter('use_theta_gate', True)
        self.declare_parameter('theta_gate_deg', 12.0)

        # Anti-flip + stabilization
        self.declare_parameter('x_ref_swap', 2.5)
        ...
        # Centerline sampling
        self.declare_parameter('center_x_start', 0.5)
        self.declare_parameter('center_x_step', 0.5)
        self.declare_parameter('center_x_count', 15)
        ...
        # AJOUT : Paramètres pour contrôle par état
        self.declare_parameter('state_control_enabled', True)
        self.declare_parameter('disabled_states', ['TURNING_180', 'ALIGNING', 'END_OF_ROW'])
        ...
        # AJOUT : Chargement des paramètres d'état
        self.state_control_enabled = bool(self.get_parameter('state_control_enabled').value)
        self.disabled_states = self.get_parameter('disabled_states').value
        ...
        self.current_nav_state = "FOLLOWING_ROW"
        self.processing_enabled = True
        
        # AJOUT : Subscriber pour l'état de navigation
        if self.state_control_enabled:
            ...
            self.get_logger().info(f"State control ENABLED. Disabled states: {self.disabled_states}")
        else:
            self.get_logger().info("State control DISABLED - processing always active")
        ...
        self.get_logger().info(
            ...
            f"  State control: {self.state_control_enabled}"
        )
    
    # AJOUT : Nouvelle méthode pour gérer les états
    def state_callback(self, msg: String):
        """Callback pour recevoir l'état de navigation"""
        old_state = self.current_nav_state
        self.current_nav_state = msg.data
        
        # Déterminer si on doit activer/désactiver le processing
        was_enabled = self.processing_enabled
        self.processing_enabled = self.current_nav_state not in self.disabled_states
        
        # Log si changement
        if old_state != self.current_nav_state or was_enabled != self.processing_enabled:
            status = "ENABLED" if self.processing_enabled else "DISABLED"
            self.get_logger().info(
                f"Navigation state: {self.current_nav_state} -> RANSAC Processing {status}"
            )
    ...
    def cb(self, msg: PointCloud2):
        ...
        # AJOUT : Check si processing désactivé
        if self.state_control_enabled and not self.processing_enabled:
            # Utiliser la dernière trajectoire valide (hold)
            centers = self.planner.hold_centers_if_needed([])
            
            # Publier les markers avec indication "DISABLED"
            self.marker_pub.publish_markers(
                msg.header, 
                None,  # Pas de ligne gauche
                None,  # Pas de ligne droite
                centers,  # Derniers centres valides
                0, 0,  # Pas d'inliers
                obstacle_side=self.planner.obs_side_mem,
                pts_count=len(filtered)
            )
            
            # Log périodique pour debug
            if not hasattr(self, '_last_disabled_log'):
                self._last_disabled_log = 0
            now_ns = self.get_clock().now().nanoseconds
            if (now_ns - self._last_disabled_log) > 2e9:  # Log toutes les 2 secondes
                self.get_logger().debug(
                    f"RANSAC disabled in state: {self.current_nav_state}, holding last trajectory"
                )
                self._last_disabled_log = now_ns
            
            return  # IMPORTANT : sortir ici pour éviter RANSAC !

        # === TOUT LE RESTE DU CODE ORIGINAL INCHANGÉ ===
